...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_30%),linear-gradient(135deg,#f8fafc,#eef2ff_45%,#ecfeff)] text-slate-950">
      <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
        <div className="flex items-center gap-3">
          <img src="/nexorder-logo.svg" alt="NexOrder AI" className="h-12 w-12 rounded-2xl shadow-sm" />
          <div>
            <p className="text-2xl font-black tracking-tight">NexOrder AI</p>
            <p className="text-sm font-semibold text-slate-500">AI order automation for Cin7 Core</p>
          </div>
        </div>
        <Link className="rounded-2xl bg-blue-600 px-6 py-3 font-black text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-700" href="/login">Login</Link>
      </header>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:py-20">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-xl leading-8 text-slate-600">NexOrder AI reads order emails, PDFs, spreadsheets, Word files and screenshots, then creates Cin7-ready sales orders so teams spend less time on manual entry and more time serving customers.</p>
        ...
        <div className="rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-2xl shadow-slate-300/50 backdrop-blur">
          <div className="rounded-3xl bg-slate-950 p-5 text-white shadow-inner">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                <p className="text-sm text-slate-400">Incoming order</p>
            ...
            <div className="mt-5 space-y-3 text-sm">
              ...
              <div className="rounded-2xl bg-white/10 p-4"><span className="text-slate-400">Products</span><p className="font-bold">Matched by SKU, name, aliases and history</p></div>
              <div className="rounded-2xl bg-white/10 p-4"><span className="text-slate-400">Result</span><p className="font-bold text-cyan-300">Sales order created in Cin7</p></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-6 pb-16 md:grid-cols-3">
        {[
          ...
          ['Improve processing speed', 'Move from email to Cin7 order in minutes instead of manual retyping.'],
          ['Support messy formats', 'Emails, PDFs, spreadsheets, Word documents and screenshots.']
        ].map(([title, text]) => (
