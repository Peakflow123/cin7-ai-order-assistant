...
export default function OutlookInboxClient({ connections }: { connections: OutlookConnection[] }) {
  ...
  async function loadInbox() {
    ...
    const response = await fetch(`/api/outlook/inbox?${params.toString()}`);
  ...
  async function processMessage(messageId: string, force = false) {
    ...
    if (data.alreadyProcessed) { setStatus(data.message || 'This Outlook message/conversation has already been processed.'); await loadInbox(); return; }
  ...
  return (
    <section className="card space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <div className="mt-7"><label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={useAiClassification} onChange={(event) => setUseAiClassification(event.target.checked)} /> Use AI classification on load</label></div>
      </div>
