...
export default function OutlookInboxClient({ connections }: { connections: OutlookConnection[] }) {
  ...
  async function loadInbox() {
    ...
    setLoading(true);
    setStatus(classifyOnLoad ? 'Loading Outlook messages with AI classification...' : 'Loading Outlook messages...');
    ...
    setStatus(`Loaded ${(data.messages || []).length} Outlook emails.`);
    setLoading(false);
  }
  ...
  return (
    <section className="card space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div><h2 className="text-xl font-black">Outlook Order Inbox</h2><p className="text-sm text-slate-500">Load recent Outlook emails and process selected customer orders.</p></div>
