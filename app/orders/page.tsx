...
export default async function Orders({ searchParams }: { searchParams?: { source?: string; account?: string } }) {
    ...
    <main className="page-shell space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="page-subtitle">Filter orders by channel and connected inbox/account.</p>
        </div>
        ...
        <form className="grid gap-3 md:grid-cols-3" action="/orders">
          ...
          <label>
            ...
            <select className="input mt-1" name="account" defaultValue={account}>
              ...
              {accounts.filter((item) => item.sourceAccount).map((item) => <option key={item.sourceAccount || ''} value={item.sourceAccount || ''}>{item.sourceAccount} ({item.source})</option>)}
            </select>
          </label>
          <div className="flex items-end gap-2">
            <button className="btn" type="submit">Apply Filters</button>
            <Link className="btn-secondary" href="/orders">Clear</Link>
          </div>
        </form>
      </section>
          ...
          {orders.map((order) => (
            <Link key={order.id} href={`/orders/${order.id}`} className="flex flex-col gap-3 rounded-2xl border border-slate-200 p-4 transition hover:bg-slate-50 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">{order.customerText || 'Customer not found'} • {order.source}{order.sourceAccount ? ` • ${order.sourceAccount}` : ''}</p>
              </div>
