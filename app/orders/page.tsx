...
export default async function Orders({ searchParams }: { searchParams?: { source?: string; account?: string; deleted?: string; from?: string; to?: string; q?: string } }) {
  ...
  if (q) {
    where.OR = [
      { customerText: { contains: q, mode: 'insensitive' } },
      ...
      <main className="page-shell space-y-6">
        <section className="client-hero">
          <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div><Link href="/dashboard" className="text-sm font-bold text-blue-700 hover:text-blue-900">Back to Dashboard</Link><h1 className="page-title mt-2">Orders</h1><p className="page-subtitle">Search and filter Gmail and Outlook orders by customer, reference, channel, mailbox and date.</p></div>
        ...
        <section className="card space-y-3">
          <form className="grid gap-3" action="/orders">
            <label>
              <span className="section-label">Search</span>
              <input className="input mt-1" type="text" name="q" defaultValue={q} placeholder="Search by customer name or order reference / PO number" />
            </label>
