...
export default function OrderPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (term.length < 2) { setSearchedProducts([]); return; }
    const timer = window.setTimeout(async () => {
      const response = await fetch(`/api/products/search?q=${encodeURIComponent(term)}`);
      const data = await response.json().catch(() => ({}));
      if (response.ok) setSearchedProducts(data.products || []);
    }, 250);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (term.length >= 2 && searchedProducts.length > 0) return searchedProducts.slice(0, 500);
  ...
  }, [products, searchedProducts, productSearch]);
  ...
  async function createCin7Order() {
    ...
    const response = await fetch(`/api/orders/${order.id}/create-cin7`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reviewFeedback }) });
  ...
  return (
    <main className="page-shell space-y-6">
      <div className="hero-card flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2"><span className={statusBadge(order.status)}>{order.status}</span>{order.cin7SaleId && <span className="badge badge-green">Cin7 Created</span>}</div>
      </div>

      <div className="grid gap-6 lg:grid-cols-3">
        <section className="card space-y-4 lg:col-span-2">
          <h2 className="text-xl font-black">Customer & Reference</h2>
          <label className="block"><span className="section-label">Mapped Cin7 Customer</span><select className="input mt-1" value={order.customerId || ''} disabled={!canEdit} onChange={(event) => setOrder({ ...order, customerId: event.target.value || null })}><option value="">No customer selected</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}{customer.email ? ` - ${customer.email}` : ''}</option>)}</select><p className="mt-1 text-xs text-slate-500">AI read customer as: {order.customerText || 'Not found'}</p></label>
          <label className="block"><span className="section-label">PO / Reference</span><input className="input mt-1" value={order.poNumber || ''} disabled={!canEdit} onChange={(event) => setOrder({ ...order, poNumber: event.target.value })} /></label>
        </section>
        ...
        <aside className="card space-y-3">
          ...
          <label className="block"><span className="section-label">Optional review feedback for AI learning</span><textarea className="input mt-1 min-h-[90px]" disabled={!canCreate} value={reviewFeedback} onChange={(event) => setReviewFeedback(event.target.value)} placeholder="Example: Customer wrote carton but matching product should be the box SKU." /></label>
      ...
      <section className="card space-y-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-black">Order Lines</h2><p className="text-sm text-slate-500">Search products by SKU or name. Search now checks the database, not just the visible dropdown list.</p></div><input className="input md:max-w-sm" placeholder="Search products by SKU or name..." value={productSearch} onChange={(event) => setProductSearch(event.target.value)} /></div>
        <div className="space-y-4">
          {order.lines.map((line) => (
            <div key={line.id} className="rounded-3xl border border-slate-200/70 bg-white/70 p-4 shadow-sm">
              <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><div><p className="text-sm font-semibold text-slate-500">Customer wrote</p><p className="font-bold text-slate-950">{line.rawProductText}</p></div><span className={line.confidence >= 0.9 ? 'badge badge-green' : line.confidence >= 0.7 ? 'badge badge-yellow' : 'badge badge-red'}>{Math.round(line.confidence * 100)}% confidence</span></div>
              <div className="grid gap-3 md:grid-cols-3"><label className="md:col-span-2"><span className="section-label">Mapped Product / SKU</span><select className="input mt-1" value={line.productId || ''} disabled={!canEdit} onChange={(event) => updateLine(line.id, { productId: event.target.value || null })}><option value="">No product selected</option>{filteredProducts.map((product) => <option key={product.id} value={product.id}>{product.sku ? `${product.sku} - ` : ''}{product.name}</option>)}</select></label><label><span className="section-label">Quantity</span><input className="input mt-1" type="number" value={line.quantity} disabled={!canEdit} onChange={(event) => updateLine(line.id, { quantity: Number(event.target.value) })} /></label></div>
      ...
      {showDelete && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-6 backdrop-blur-sm"><div className="card max-w-xl space-y-4"><h2 className="text-2xl font-black text-slate-950">Delete this review order?</h2><p className="text-sm text-slate-600">This will remove the review order and save your feedback so AI can learn not to classify similar emails as orders in the future.</p><label className="block"><span className="section-label">Why is this not a real order?</span><textarea className="input mt-1 min-h-[120px]" value={deleteComment} onChange={(event) => setDeleteComment(event.target.value)} placeholder="Example: This is a supplier purchase order, not a customer sales order. Do not process similar emails." /></label><div className="flex justify-end gap-2"><button className="btn-secondary" onClick={() => setShowDelete(false)} disabled={isDeleting}>Cancel</button><button className="btn-danger" onClick={deleteOrder} disabled={isDeleting}>{isDeleting ? 'Deleting...' : 'Delete and Save Feedback'}</button></div></div></div>}
    </main>
  );
}
