...
export default async function AdminDashboard() {
  ...
  const [companies, totalOrders, users, needsReview, created, errors, gmail, outlook, feedback] = await Promise.all([
    ...
    <main className="page-shell">
      <div className="admin-shell">
        <aside className="admin-sidebar space-y-5">
          <div><p className="badge badge-blue">Platform control</p><h1 className="mt-3 text-2xl font-black tracking-tight text-slate-950">Admin Dashboard</h1><p className="mt-2 text-sm leading-6 text-slate-500">Control client access, users, connection limits and automation rules.</p></div>
          <nav className="space-y-2"><a className="nav-link-active block" href="#overview">Overview</a><a className="nav-link block" href="#clients">Client Controls</a><Link className="nav-link block" href="/admin/users">Users</Link><Link className="nav-link block" href="/admin/clients">Client List</Link></nav>
        </aside>

        <section className="space-y-6">
          <section id="overview" className="hero-card flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div><h2 className="page-title">Command Center</h2><p className="page-subtitle">Separate admin area for launch control, test account cleanup and client limits.</p></div>
            <div className="flex gap-2"><Link className="btn" href="/admin/users">Manage Users</Link><Link className="btn-secondary" href="/dashboard">Client Workspace</Link></div>
          </section>
          ...
          <section id="clients" className="space-y-4">
            <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between"><div><h2 className="text-2xl font-black tracking-tight text-slate-950">Client Controls</h2><p className="text-sm text-slate-500">Manage activation, Gmail/Outlook limits, Cin7 edit permission and automation threshold.</p></div><Link className="btn-secondary" href="/admin/clients">Compact client list</Link></div>
