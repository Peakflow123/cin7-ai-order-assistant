...
export default function AdminCin7SyncButton({ companyId, hasCin7 }: { companyId: string; hasCin7: boolean }) {
  ...
  async function refreshEntity(entity: 'Product' | 'Customer', totals: any) {
    for (let page = 1; page <= 200; page += 1) {
      const data = await postJson(`/api/admin/companies/${companyId}/cin7-refresh-step`, {
        entity,
        page,
        since: null
      });
  ...
  return (
    <button className="btn-secondary px-3 py-2 text-sm" disabled={syncing} onClick={refresh}>
