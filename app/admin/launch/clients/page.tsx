...
export default async function LaunchClientsPage() {
  ...
  return (
    <AdminPortalShell title="Clients & Controls" subtitle="Manage company status, archive state, pricing plan, connection limits and permanent deletion from one clean client control page.">
          ...
          <section key={client.id} className="card space-y-4">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">Orders: {client.orders} • Needs review: {client.needsReview} • Errors: {client.errorOrders}</p>
              </div>
