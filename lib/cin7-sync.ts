...
async function fetchPaged(connection: any, endpoint: string, since?: Date | null) {
  const all: any[] = [];
  const limit = 500;
  ...
  for (let page = 1; page <= 40; page += 1) {
    ...
    // This is incremental preference. If Cin7 ignores this parameter, upsert still prevents duplicate local records.
...
async function fetchWithFallback(connection: any, endpoints: string[], since?: Date | null) {
  let lastError: any;

  for (const endpoint of endpoints) {
    try {
      return await fetchPaged(connection, endpoint, since);
    } catch (error) {
      lastError = error;
    }
  }
...
async function syncCustomers(companyId: string, connection: any, since?: Date | null) {
  ...
  for (const customer of rows) {
    ...
    await prisma.customer.upsert({
      where: { companyId_cin7Id: { companyId, cin7Id } },
      update: { name: customer.Name || customer.CustomerName || 'Unnamed Customer', email, phone },
      create: { companyId, cin7Id, name: customer.Name || customer.CustomerName || 'Unnamed Customer', email, phone }
    });
...
export async function syncCin7Company(companyId: string, options: SyncOptions) {
  ...
  const lastSyncMessage = `Products: ${products.created} created, ${products.updated} updated, ${products.skipped} skipped. Customers: ${customers.created} created, ${customers.updated} updated, ${customers.skipped} skipped.`;
  ...
  await logActivity({
    ...
    message: `Cin7 sync completed. ${lastSyncMessage}`,
    metadata: { products, customers, mode: options.mode, incremental: true }
  });

  return { products, customers, incremental: true, syncedAt: startedAt.toISOString(), message: lastSyncMessage };
}
